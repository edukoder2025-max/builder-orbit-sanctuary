...
function writeMarkdown({ title, content, tags, excerpt }: { title: string; content: string; tags: string[]; excerpt: string }) {
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      articles = parts.map((p, i) => ({ title: `Artículo ${i+1}`, content: p, tags: ['general'], excerpt: p.slice(0, 160) }));
    }
    const created = articles.slice(0, 10).map((a) => {
      const title = String(a.title || 'Artículo');
      const content = String(a.content || a.body || '');
      const tags = Array.isArray(a.tags) ? a.tags : (String(a.tags||'').split(',').map((s:string)=>s.trim()).filter(Boolean));
